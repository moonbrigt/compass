...
add(0, "今日笔记和每日问题提示词已设置快捷键", (() => { try { const hk = app.hotkeyManager.customKeys || {}; return ["quickadd:choice:lifeos-daily", "templater-obsidian:Templates/Daily Questions Prompt.md"].every(id => (hk[id] || []).length > 0); } catch (e) { return false; } })(), "设置 → 快捷键");
...
add(1, "已将理想一周改为自己的安排（移除示例属性）", !((dv.page("03 Planning/Ideal Week") || {}).example === true), "[[Ideal Week]]", "填写表格后删除 example 属性");
...
add(2, "本周笔记已创建", !!dv.page(`${cfg.weekly_folder || "01 Journal/Weekly"}/${today.format("gggg-[W]ww")}`), "命令面板：Periodic Notes: Open weekly note", "从第 2 周开始");
const retreat = dv.page(`${cfg.retreat_folder || "02 Retreats"}/${today.format("YYYY-[Q]Q")} Personal Retreat`);
...
if (app.vault.getAbstractFileByPath("09 Reading")) add(2, "已决定是否使用阅读模块（填写计划或删除文件夹）", /^- \[ \]/m.test(plan), "[[07 Workflow - Daily Reading]]", "可选");
