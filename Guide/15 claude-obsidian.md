...
它不负责运行 Compass。事务核心只写入 `wiki/` 与 `.raw/`；日记、静修、任务、人物和写作仍在 Obsidian 中用 QuickAdd、Templater、Tasks、Kanban，或经 Agent Client 逐次批准后修改，见 [[14 Agent Client and Claude Code|Agent Client 与 Claude Code]]。两层都读取仓库根目录的 `CLAUDE.md`。
