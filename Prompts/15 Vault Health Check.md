...
type: button
text: "检查仓库健康状况"
prompt: "请用 vault_read 阅读 Prompts/15 Vault Health Check.md，并针对当前打开的笔记执行其中的“## 提示词”部分；如果当前笔记不适用，则使用当前时间段。"
viewType: right-pane
```
...
1. 示例内容：调用 tag_list，找出带 `example` 标签的笔记（可用 search_simple 搜索 `tag:#example`，或读取 frontmatter），按文件夹列出。另在 `01 Journal/Daily` 中搜索“示例记录”和“示例收获”。
2. 占位内容：vault_read `Meta/Compass Config.md`，标记空白的 `birthdate`；vault_read `03 Planning/Life Theme.md` 与 `Core Values.md`，标记仍保留“请在此写下你的生活主题”或“**价值一**”等模板文字的笔记；vault_read `03 Planning/Ideal Week.md`，标记 `example: true`；vault_read `08 Tasks/Tasks.md`，标记尚未完成的 Setup 任务。
...
4. 属性偏差：读取 `01 Journal/Daily` 的每篇笔记，确认每个 `dq_*` 值为空或 1 到 10 的整数，每个 `habit_*` 值为 `true` 或 `false`；列出不符合的项。确认 `02 Retreats` 中每篇笔记都有 `wheel_*` 属性，且文件名符合 `YYYY-QN Personal Retreat`。
