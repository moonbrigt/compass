...
1. vault_read `01 Journal/Weekly/<本周 gggg-Www>.md`，读取 `## 本周意向`。如果今日笔记存在，也 vault_read `## 日记`，以我的原话了解精力和背景。
2. 收集未完成任务：vault_read `08 Tasks/Tasks.md`；用 search_simple 在仓库中搜索 `📅 `、`⏳ ` 和 `⏫`，排除 `wiki/` 与 `09 Reading`；对 `status` 为 `active` 的 `04 Projects` 笔记，vault_read 其 `## 笔记内任务`；用 search_simple 搜索 `#discuss`，找出等待今天会面的事项。只保留未勾选的任务行。
