...
add(1, "已填写生活主题", theme.length > 0 && !theme.includes("请在此写下你的生活主题"), "[[人生主题]]");
const values = await readText("03 规划/核心价值观.md");
add(1, "已填写核心价值观", values.length > 0 && !/\*\*价值一\*\*/.test(values), "[[核心价值观]]");
add(1, "已将理想一周改为自己的安排（移除示例属性）", !((dv.page("03 规划/理想一周") || {}).example === true), "[[理想一周]]", "填写表格后删除 example 属性");
add(1, "已检查每日问题、习惯和生活领域", Array.isArray(cfg.questions) && cfg.questions.length > 0 && Array.isArray(cfg.habits) && cfg.habits.length <= 5, "[[Compass 配置]]", Array.isArray(cfg.habits) && cfg.habits.length > 5 ? "习惯超过 5 项；建议每阶段保持 3 到 5 项" : "");
const examples = dv.pages("#example").length;
add(1, "已删除示例笔记", examples === 0, "[[16 入门助手]] 第 6 步，或删除带 example 标签的笔记", examples ? `还剩 ${examples} 篇示例笔记` : "");
...
add(2, "已回答至少一次真实的每日问题", answered.length > 0, "今晚按 Ctrl/Cmd+Shift+Q，或使用 [[02 日终复盘辅导]]");
...
const retreat = dv.page(`${cfg.retreat_folder || "02 静修"}/${today.format("YYYY-[Q]Q")} 个人静修`);
add(2, "本季度个人静修笔记已创建", !!retreat && !(retreat.tags || []).includes("example"), "[[04 工作流 - 个人静修]]", "从第 60 天开始");
...
add(3, "已为代理注册 Obsidian MCP 服务器（手动确认）", cur.setup_mcp_registered === true, "阅读 [[19 Obsidian MCP 桥接]]，再勾选 setup_mcp_registered", "可选");
add(3, "Agent Client 已进行过对话", (ac?.savedSessions || []).length > 0, "[[智能助手]]", "可选");
...
add(4, "Vault Lens 扩展已连接（手动确认）", cur.setup_vault_lens === true, "阅读 [[17 搜索服务]]，再勾选 setup_vault_lens", "可选");
