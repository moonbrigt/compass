...
birthdate:
life_expectancy: 80
daily_folder: 01 日记/每日
weekly_folder: 01 日记/每周
quarterly_folder: 01 日记/季度
retreat_folder: 02 静修
...
这是系统读取配置的统一入口。`元数据/视图/` 中的仪表盘组件使用 `dv.page("元数据/Compass Config")` 读取本页，新建日记、个人静修和每日问题时也会读取下方列表。在这里修改配置即可，无需移动其他文件。
