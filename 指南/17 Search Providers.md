...
5. 等待绿色“连接成功”提示。
6. 用 `指南/00 Start Here.md` 中的一个词进行网页搜索，检查扩展图标是否变绿并列出该笔记。
7. 在扩展侧栏点日记按钮，检查新笔记是否位于 `01 日记/每日/`。若笔记为空且缺少属性，可对活动文件运行一次“Templater: Replace templates in the active file”；QuickAdd 捕获命令仍可使用。
...
交付模板中的 Local REST API 设置仅包含 `{"enableInsecureServer": true}`，不附带生成的密钥或证书。首次运行时由插件在本机生成凭据；不要把凭据写进仓库或分享给他人。
