...
tags:
  - retreat
<%* const _cf = app.vault.getAbstractFileByPath("元数据/Compass 配置.md"); const _cfg = _cf ? (app.metadataCache.getFileCache(_cf)?.frontmatter ?? {}) : {}; const _ws = Array.isArray(_cfg.wheel_areas) && _cfg.wheel_areas.length ? _cfg.wheel_areas : ["wheel_health","wheel_relationships","wheel_family","wheel_career","wheel_finances","wheel_growth","wheel_fun","wheel_meaning"]; tR += _ws.map(k => k + ": ").join("\n"); %>
---
> 请将本笔记命名为 `YYYY-QN 个人静修`，例如 `2026-Q3 个人静修`。Compass 仪表盘会按这个命名规则找到本季度的静修笔记，并使用上方的 `wheel_*` 属性绘制生活之轮；无需修改代码。

上次静修：[[02 静修/<% moment(tp.file.title.slice(0, 7), "YYYY-[Q]Q").subtract(1, "quarter").format("YYYY-[Q]Q") %> 个人静修|上季度个人静修]] · 季度笔记：[[01 日记/季度/<% tp.file.title.slice(0, 7) %>|本季度笔记]] · 去年同期：[[02 静修/<% moment(tp.file.title.slice(0, 7), "YYYY-[Q]Q").subtract(1, "year").format("YYYY-[Q]Q") %> 个人静修|去年同期个人静修]]
