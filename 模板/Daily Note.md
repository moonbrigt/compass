...
tags:
  - daily
<%* const _cf = app.vault.getAbstractFileByPath("元数据/Compass Config.md"); const _cfg = _cf ? (app.metadataCache.getFileCache(_cf)?.frontmatter ?? {}) : {}; const _qs = Array.isArray(_cfg.questions) && _cfg.questions.length ? _cfg.questions.map(q => (q && q.key) ? q.key : q).filter(Boolean) : ["dq_goals","dq_progress","dq_meaning","dq_happy","dq_relationships","dq_engaged"]; const _hs = Array.isArray(_cfg.habits) && _cfg.habits.length ? _cfg.habits : ["habit_journal","habit_exercise","habit_reading"]; tR += _qs.map(k => k + ": ").join("\n") + "\n" + _hs.map(k => k + ": false").join("\n"); %>
---
« [[01 日记/每日/<% tp.date.now("YYYY-MM-DD", -1, tp.file.title, "YYYY-MM-DD") %>|昨天]] · [[01 日记/每周/<% tp.date.now("gggg-[W]ww", 0, tp.file.title, "YYYY-MM-DD") %>|本周]] · [[01 日记/季度/<% tp.date.now("YYYY-[Q]Q", 0, tp.file.title, "YYYY-MM-DD") %>|本季度]] · [[Compass Dashboard|Compass 仪表盘]] · [[01 日记/每日/<% tp.date.now("YYYY-MM-DD", 1, tp.file.title, "YYYY-MM-DD") %>|明天]] »
...
> ```tasks
> not done
> path includes 09 阅读/Reading Plan
...
> [!intention]- 本周意向
> ![[01 日记/每周/<% tp.date.now("gggg-[W]ww", 0, tp.file.title, "YYYY-MM-DD") %>#本周意向]]
...
## 每日问题
今晚按每日问题快捷键 Ctrl/Cmd+Shift+Q，或运行命令 **Templater: Insert 模板/Daily Questions Prompt.md**。它会按 [[Compass Config|Compass 配置]] 中的列表提问，并填写上方的 `dq_*` 和 `habit_*` 属性。用 1 到 10 分评价努力程度，而非结果。
