...
/*
  ...
  问题来自 元数据/Compass 配置.md 的 `questions` 列表；仅在缺少该列表时使用下方 FALLBACK。
...
const cfg = app.metadataCache.getFileCache(app.vault.getAbstractFileByPath("元数据/Compass 配置.md"))?.frontmatter || {};
