...
维护者在自己的仓库中保存真实笔记。本仓库保存构建后的模板，而非个人使用中的仓库。`scripts/build_template.py` 从源仓库复制内容时，会排除 Git 状态、`.vault-meta/`、`.mcp.json`、`.claude/settings.local.json`、工作区文件、Agent Client 会话与导出、附件、`wiki/` 内容目录和 `inbox/` 内容；用户目录只保留带 `example` 标签的笔记。它恢复默认出生日期、`03 规划/*`、插件设置，加入 `元数据/version.md` 和简易工作区，再运行验证和压缩。`scripts/RELEASE.md` 列出发布检查。
...
1. **系统文件在源仓库中修改，再通过构建脚本发行。** 仪表盘、`元数据/视图/`、`指南/`、`模板/`、`提示词/`、`scripts/`、`AGENTS.md` 和插件设置都应进入源仓库。不要只在构建副本中修改需要保留到下次发行的内容。
...
3. **拉取请求前运行验证。** 在仓库根目录执行 `python3 scripts/verify_template.py .`，需要 Python 3 和 Node（检查 `元数据/视图/*.js` 语法）。GitHub Actions 的 `verify` 流程也会在每次推送和拉取请求时运行。
4. **不要使用英文长破折号 U+2014。** 验证器会拒绝文本、代码、注释与提示词中的该字符；可改用逗号、句号、冒号或括号。
5. `THIRD_PARTY_NOTICES.md`、`元数据/version.md` 与 `.obsidian/plugins/` 中的插件清单和版本保持一致；验证器会检查。
