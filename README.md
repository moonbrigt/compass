<p align="center"><img src="Meta/attachments/cover.svg" alt="Compass 简体中文版封面" width="100%"></p>
...
**状态：简体中文本地候选，尚未公开发布。** 核心桌面流程已在 Obsidian 1.13.7 中试用；移动端、真实 AI 请求和本机 API 桥接尚未验收。要求 Obsidian 1.13.1 或更新版本。实际检查结果见仓库开发分支的 `specs/001-simplified-chinese-vault/acceptance.md`。
...
4. 阅读[入门指南](Guide/00%20Start%20Here.md)，打开 `00 Dashboards/Setup.md`，按照自检提示完成设置。
...
```text
00 Dashboards/   设置、Compass、习惯、每日问题、任务、项目、看板、助手仪表盘
01 Journal/      每日、每周、每季度笔记
02 Retreats/     YYYY-QN Personal Retreat 个人静修笔记，属性含生命之轮分数
03 Planning/     人生主题、核心价值观（含角色）、理想一周
04 Projects/     项目笔记、#project/<slug> 任务和项目看板
05 People/       人物笔记、#p/<slug> 任务及 #discuss 待讨论事项
06 Writing/      通讯、视频脚本、文章、课程内容及各自看板
07 Library/      读书笔记，可用块 ID 嵌入引文
08 Tasks/        Tasks.md，任务总表
09 Reading/      阅读计划、章节、经节、研读笔记与主题
Prompts/         16 篇智能体提示词
Templates/       Templater 模板；属性列表来自 Meta/Compass Config.md
Meta/            唯一配置、views/*.js 仪表盘组件、版本说明
Guide/           原则、插件、各工作流、智能体、MCP 与提示词说明
...
`Meta/Compass Config.md` 集中保存问题、习惯、生命之轮领域、目录、前缀与出生日期。仪表盘按 `dq_*`、`habit_*`、`wheel_*` 前缀发现属性，因此改动配置后新笔记与相关视图会相应变化。

## 七项工作流

| 编号 | 工作流 | 主要文件 | 指南 |
| --- | --- | --- | --- |
| 1 | 日记与每日问题 | `01 Journal/Daily`、`Templates/Daily Note.md`、`Templates/Daily Questions Prompt.md`、`Meta/Compass Config.md` | `Guide/03 Workflow - Journaling and Daily Questions.md` |
| 2 | 季度个人静修 | `02 Retreats`、`Templates/Personal Retreat.md` | `Guide/04 Workflow - Personal Retreat.md` |
| 3 | 多尺度规划 | `01 Journal/{Daily,Weekly,Quarterly}`、`03 Planning` | `Guide/05 Workflow - Multi-Scale Planning.md` |
| 4 | 习惯追踪 | 日记中的 `habit_*`、`00 Dashboards/Habit Canvas.md` | `Guide/06 Workflow - Habit Tracking.md` |
| 5 | 每日阅读（以《圣经》为例） | `09 Reading` | `Guide/07 Workflow - Daily Reading.md` |
| 6 | 任务管理 | `08 Tasks/Tasks.md`、`04 Projects`、`05 People`、任务仪表盘 | `Guide/08 Workflow - Task Management.md` |
| 7 | 写作 | `06 Writing/*` 与 Kanban 看板 | `Guide/09 Workflow - Writing.md` |

此外还有 Compass 仪表盘（`Guide/10`）、看板（`Guide/13`）、仓库中的 AI（`Guide/14`、`Guide/20`）、claude-obsidian 知识层（`Guide/15`）、研究与发布工具（`Guide/16`、`Guide/17`）及 Obsidian MCP 桥接（`Guide/19`）。从[入门指南](Guide/00%20Start%20Here.md)开始阅读。每次只加一个工作流；建议先把日记稳定运行 30 天。“设置”页按此顺序提示。

## 附带插件

十个社区插件已放在 `.obsidian/plugins/` 并预设启用，各有版本及许可记录。与上游发布包逐字节核对仍是独立的发布检查。仓库自有的 `life-os-app` 与它们一同安装。首次打开检查见 `Guide/02 Plugins.md`。
...
自有的 Life OS（`life-os-app`）单独标注版本，采用 MIT 许可，负责原生导航、捕获和实时仪表盘，详见 `Guide/21 Life OS Application.md`。上游仓库和版本标签见 `THIRD_PARTY_NOTICES.md`；本仓库不附带 Obsidian 程序。

## 仓库中的智能体

- `AGENTS.md` 规定目录、属性、读取与写入边界；`CLAUDE.md`、`GEMINI.md` 指向它。
- `Prompts/` 含 16 项重复工作，每篇都有风险级别与按钮，详见 `Guide/20 Prompt Library.md`。
- Agent Client 在侧栏或笔记里运行本机智能体；自动批准默认关闭。写入前应检查差异并批准，详见 `Guide/14 Agent Client and Claude Code.md`。
- Local REST API 5.x 可在 `http://127.0.0.1:27123/mcp` 提供 16 个 MCP 工具，连接示例见 `.mcp.example.json`；Claude Code 的只读预授权见 `.claude/settings.json`。详见 `Guide/19 Obsidian MCP Bridge.md`。
...
python3 scripts/build_template.py --live . --out ../compass-builds --name Compass-zh-CN-personal --version 1.1.0-zh.6 --zip
...
仓库没有原位事务升级器。升级时先备份完整仓库，把新版本解压到旁边，再逐项审查并迁移个人笔记、配置和模板；不要整目录替换正在使用的 `.obsidian`。备份只有经过恢复演练才算已验证。参见 `scripts/RELEASE.md` 与 `Guide/23 Native Acceptance.md`。

## 致谢与许可

工作流参考 Mike Schmitz 的公开视频。每日问题参考 Marshall Goldsmith 与 Mark Reiter 的《Triggers》（2015）；多尺度规划参考 Cal Newport。完整说明见 `CREDITS.md`。

代码、模板、仪表盘、脚本和配置使用 MIT（`LICENSE`）；`Guide/` 的说明文字使用 CC BY 4.0（`LICENSE-GUIDE.md`）。`.obsidian/plugins/` 中的第三方插件保留各自许可，见 `THIRD_PARTY_NOTICES.md`。社区规则见 `CODE_OF_CONDUCT.md`；参与方式见 `CONTRIBUTING.md`；安全说明见 `SECURITY.md`。
