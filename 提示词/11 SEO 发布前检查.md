...
任务：对当前打开的写作笔记做发布前检查。
1. 调用 active_file_get_path；路径必须在 `06 写作`。vault_read 当前笔记。
...
6. 对批准的修复逐项使用 vault_patch。结束时重新运行 `seo:run-current`，并请我提供新的分数。不要移动看板卡片；当我确认作品就绪时，由 `提示词/10 写作流程.md` 处理。
```
