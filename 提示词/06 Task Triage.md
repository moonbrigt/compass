...
writes: "经每批批准后，修改 08 任务/Tasks.md 中的单条任务。"
risk: "edit"
inputs:
  - "08 任务/Tasks.md"
  - "04 项目 中的笔记名称与标签短名"
...
任务：整理任务收件箱。
1. vault_read `08 任务/Tasks.md`。收集 `## 收件箱` 下所有未完成、没有 `#project/` 标签、没有 `#p/` 标签且没有 📅 到期日期的任务行。
...
5. 经批准后，使用 vault_patch 只修改 `08 任务/Tasks.md` 中展示过的任务行；保留原有的 ➕ 日期和所有表情符号。移动到“将来再做”时，先在 `## 将来再做` 下添加，再删除收件箱中的原行。
6. 报告各类处理的数量，然后用 open_file 打开 `00 仪表盘/Task Dashboard.md`。
```
