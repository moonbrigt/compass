...
1. 调用 active_file_get_path；路径必须在 `06 写作`。vault_read 当前笔记，记录 `type`（`newsletter`、`youtube-script`、`article`、`course-lesson`）、`status`、`sources` 列表及空白章节。
2. 询问当前处于“大纲”“草稿”还是“编辑”阶段。不得只凭 `status` 推断，必须确认。
3. 对 `sources` 属性中的每篇笔记执行 vault_read。对读书笔记，列出引文及其 `^block-id`，以便用 `![[Note#^id]]` 嵌入。若 `sources` 为空，用 search_simple 在 `07 资料库` 和 `01 日记/每日` 中搜索暂定标题的关键名词，提供候选；未经同意不得向 `sources` 添加内容。
...
编辑阶段：读取完整草稿，列出具体修改建议（删减、压缩、澄清、补充来源），每项同时给出原句和拟改句。只应用我指定编号的修改。之后提醒我在作品离开仓库前运行 `提示词/11 SEO Pre-publish Audit`，并经同意后将 `status` 设为 `editing`。
